 ...
 * @author xcesco
 */
class ISSTiledLayerHandler(layer: TiledLayer?) : TiledLayerHandler(layer!!) {
    ...
    override fun drawLayer(shader: ShaderTiledMap?, deltaTime: Long, startLayerColumn: Int, startLayerRow: Int, offsetX: Int, offsetY: Int, modelview: Matrix4x4?) {
        ...
        var layerCurrentColumn: Int
        var row: Int
        startLayerRow = if (startLayerRow >= 0) startLayerRow else 0
        startLayerColumn = if (startLayerColumn >= 0) startLayerColumn else 0
        if (layer!!.oldStartLayerColumn != startLayerColumn || layer!!.oldStartLayerRow != startLayerRow) {
            ...
            for (i in 0 until windowRows) {
                ...
                row = clampI(layerCurrentRow - view!!.tileRowOffset, 0, layer!!.tileRows - 1)
                /* if (row < 0) {
                    row = 0;
                }*/
                //row = layerCurrentRow;
    ...
    override fun onBuildView(view: TiledMapView) {
        ...
        if (layer!!.tileWidthMax != layer!!.tiledMap.tileWidth || layer!!.tileHeightMax != layer!!.tiledMap.tileHeight) {
            ...
            vertexBuffer = ISSHelper.buildISSVertexBuffer(
                view.windowDimension,
                view.tileRowOffset,
                view.windowTileRows,
                view.windowTileColumns,
                layer!!.tiledMap.tileWidth.toFloat(),
